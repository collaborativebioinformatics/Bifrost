...
def overseer_decide(spec_hash: str, decision: str, body: OverseerDecision | None = None):
    ...
    try:
        d = overseer_queue.decide(spec_hash, decision == "approve", body.note, body.by)
    except KeyError:
        raise HTTPException(404, "not in queue")
    return {"spec_hash": spec_hash, "decision": "RELEASED" if decision == "approve" else "REJECTED",
            "released": (d / "released.json").exists()}
