...
FedAvg convergence (max |β − truth|): round 1 3.5e-1 → round 3 1.9e-3 → round 5 7.3e-6 → round 10 2.2e-11. With `--local-steps 5` this run reported err ≈ 2.4e-2 without recording its learning rate or round count. A controlled comparison holding both fixed and varying only `local_steps` is in [`local_steps_controlled.md`](local_steps_controlled.md), pinned by [`tests/test_local_steps_controlled.py`](../tests/test_local_steps_controlled.py): one step reaches pooled OLS, five leaves ~2.3e-2 unchanged from 10 to 200 rounds.
