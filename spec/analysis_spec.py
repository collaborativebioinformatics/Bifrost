...
class AnalysisSpec(BaseModel):
    ...
    outcome: str | None = None  # fed_linreg/fed_logreg: canonical name of the outcome; rest of `variables` are features

    @model_validator(mode="after")
    def _check(self):
        if self.analysis_type in ("fed_linreg", "fed_logreg") and not self.outcome:
            raise ValueError(f"{self.analysis_type} needs `outcome`")
